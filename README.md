...
It is tested and developed against 0.12.4; building against 0.13 is also in
progress.
...
1. Install sbt (http://www.scala-sbt.org) or macports, brew, etc.
...
3. Create a directory named `project` within your project and add a file
   `plugins.sbt`, in it, add the following lines:
    ...
    addSbtPlugin("com.hanhuy.sbt" % "android-sdk-plugin" % "0.8.1")
    ```
...
* Create instrumented test apks; no test apk support yet.
