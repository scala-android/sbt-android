...
object Tasks {
    ...
    def using[A <: { def close() },B](closeable: => A)(f: A => B):
            Option[B] = {
        var c: Option[A] = None
        try {
            c = Option(closeable)
    ...
    private def outofdate(dfile: File): Boolean = {
        (using (Source.fromFile(dfile)) { s =>
            ...
            dependencies.exists { case (d,l) =>
                val f = new File(d)
                l.exists { i =>
                    new File(i).lastModified > f.lastModified }
            }
        }) getOrElse false
    }

    def apkbuildTaskDef = ( name
                          , packageResources
                          , dex
                          , baseDirectory
                          , binPath
    ...
    def signReleaseTaskDef = (properties, apkbuild, streams) map {
        (p, a, s) =>
    ...
    def zipalignTaskDef = (zipalignPath, signRelease, streams) map {
        (z, r, s) =>
        if (r.getName.contains("-unsigned")) {
            ...
            val rv = Seq(z, "-f", "4",
                    r.getAbsolutePath, aligned.getAbsolutePath) !
    ...
    def pngCrunchTaskDef = (aaptPath, binPath, baseDirectory) map {
        (a, bin, base) =>
        val res = bin / "res"
        res.mkdirs()
        Seq(a, "crunch", "-v",
    ...
    def proguardInputsTaskDef = ( proguardScala
                                , proguardLibraries
                                , proguardExcludes
                                , managedClasspath in Compile
    ...
    def proguardTaskDef: Project.Initialize[Task[Option[File]]] =
        ...
        if (p && !l) {
            ...
            if ((jars ++ libjars).exists {
                    _.lastModified > t.lastModified }) {
    ...
    def unmanagedJarsTaskDef = ( unmanagedJars
                               , baseDirectory
